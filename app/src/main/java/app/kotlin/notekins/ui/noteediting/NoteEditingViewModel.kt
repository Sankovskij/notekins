...
class NoteEditingViewModel(val notesRepository: NotesRepository) : ViewModel(), CoroutineScope {
    ...
    suspend fun saveNote(note: Note, title: Editable?, text: Editable, color: Note.Color) {
        launch {
            title?.let {
                if (it.length < 3) return@launch
            }
    ...
    fun deleteNote() {
        launch {
            editingNote.let { notesRepository.delete(it) }
